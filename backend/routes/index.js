...
// lấy danh mục
router.post('/addCategorie', upload.single('image'), async (req, res, next) => {
  const db = await connectDb();
  const categoryCollection = db.collection('categories');
  const { name,status,describe } = req.body;
  const image = req.file.originalname;
  const newCategory = { name,status, image ,describe};
...
router.delete('/deletecategory/:id', async (req, res, next) => {
  const db = await connectDb();
  const catelogyCollection = db.collection('categories');
  const id = new ObjectId(req.params.id);
  try {
    const result = await catelogyCollection.deleteOne({ _id: id });
    if (result.deletedCount) {
      res.status(200).json({ message: "Xóa sản phẩm thành công" });
    } else {
      res.status(404).json({ message: "Không tìm thấy sản phẩm" });
...
router.put('/updatecategory/:id', upload.single('image'), async (req, res, next) => {
  ...
  let updatedCatelory = { name,describe }; 

  if (req.file) {
    const image = req.file.originalname;
    updatedCatelory.image = image; //
  }

  try {
    const result = await CategoryCollection.updateOne({ _id: id }, { $set: updatedCatelory });
    if (result.matchedCount) {
      res.status(200).json({ message: "Sửa sản phẩm thành công" });
    } else {
      res.status(404).json({ message: "Không tìm thấy sản phẩm" });
